...
pub struct Config {
    ...
    /// The directory the lane's file lives in. Fleet callers never assemble
    /// this: [`Config::for_module`] derives it from the module id through the
    /// same resolver every store uses, so a log cannot land beside the wrong
    /// data. The field is public for tests and for the daemon's own
    /// `run/logs/` lane, which is not a module data directory.
...
impl Config {
    /// The fleet configuration for a supervised module's own process: the file
    /// is `<module data dir>/logs/<module_id>.log`, `CK_LOG` comes from the
    /// environment (the daemon injects it at spawn), retention is the fleet
    /// default. A plugin lane is the same call with [`Lane::Plugin`].
    pub fn for_module(module_id: &str, lane: Lane) -> Self {
