...
/// The fleet constructor derives the directory from the same resolver every
/// store uses, so a module cannot log beside the wrong data directory.
#[test]
fn for_module_places_the_file_under_the_module_data_dir() {
    let config = Config::for_module("magic-context", Lane::Plugin("opencode".into()));
    let expected =
        PathBuf::from(cortexkit_store_types::module_data_dir("magic-context")).join("logs");
